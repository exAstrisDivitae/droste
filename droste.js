...
/*
    ...
    // THIS FN CREATES AN INFINITE LOOP BC THE LIMIT APPROACHES ZERO INFINITELY
    effect(k) {
        do {
            this._limit(k)
        } while (this._volume(k) !== 0);
        return console.log(this._limit(k));
    }
*/
    // This fn returns an approximation of the limit
    // based on a finite number of scaling functions.
    // The greater the value of i, the closer the
    // aproximation of the limit is to the actual limit.
    effect(k) {
        let i = 0;
        do {
            i += 1
    ...
    logResults() {
        console.log(`Limit ~= ${this._integral}`);
